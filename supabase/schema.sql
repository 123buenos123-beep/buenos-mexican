...
-- logging tables: these hold PII too — booking_attempts has customer name+email,
-- email_logs holds every blast recipient (the whole subscriber list). Anon gets
-- no read access anywhere. Drop EVERY existing policy first (defensive — clears
-- the legacy open-access policies under any name).
DO $$
DECLARE pol record;
BEGIN
...
-- booking_attempts: written only by the server bookings route (service_role,
-- bypasses RLS); read/cleared by the authenticated admin dashboard.
...
-- email_blasts / email_logs: written by the newsletter send route as the
-- authenticated admin and by the Resend webhook via service_role; read by the
-- admin dashboards.
